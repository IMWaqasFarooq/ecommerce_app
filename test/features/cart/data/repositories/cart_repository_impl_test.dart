  ...
  group('as a guest', () {
    ...
    test('addItem increments quantity when the product is already in the cart', () async {
      when(() => localDataSource.getItems('guest')).thenReturn([
        const CartItemModel(productId: 1, title: 'Mascara', thumbnail: 't', price: 9.99, quantity: 2),
      ]);
      when(() => localDataSource.getCoupon('guest')).thenReturn(null);
      when(() => localDataSource.saveItems('guest', any())).thenAnswer((_) async {});

      final result = await repository.addItem(productId: 1, title: 'Mascara', thumbnail: 't', price: 9.99);
    ...
    test('applyCoupon rejects an unknown code without touching storage', () async {
      ...
      expect(result, const Left<Failure, dynamic>(Failure.validation(message: 'Invalid coupon code')));
    ...
    test('applyCoupon persists a known code', () async {
      ...
      when(() => localDataSource.getCoupon('guest'))
          .thenReturn(const CouponModel(code: 'SAVE10', discountPercentage: 10));

      final result = await repository.applyCoupon('save10');

      expect(result.isRight(), isTrue);
      verify(() => localDataSource.saveCoupon('guest', const CouponModel(code: 'SAVE10', discountPercentage: 10)))
          .called(1);
    });
  });

  group('mergeGuestCartIntoUser', () {
    test('sums quantities for shared products and keeps unique ones, then clears the guest cart', () async {
      ...
      final captured =
          verify(() => localDataSource.saveItems('user-1', captureAny())).captured.single as List<CartItemModel>;
